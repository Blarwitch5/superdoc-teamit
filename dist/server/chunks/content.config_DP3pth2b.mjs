import { L as LIVE_CONTENT_TYPE, C as CONTENT_LAYER_TYPE, g as getCollectionPathFromRoot, n as defineCollection } from './translations_Bzfg-7-g.mjs';
import './middleware_98rRTmN7.mjs';
...
import { g as getDefaultExportFromCjs, c as commonjsGlobal } from './server_DDFmLy_E.mjs';
import { aL as slash, d as s, a_ as object, aY as string, aW as union, bl as array, b1 as record, b3 as never, bK as any, bL as literal, bM as _function, bN as promise$1, b9 as custom, bO as NEVER } from './sequence_DUL7TRkU.mjs';
import './deterministic-string_DH2Jdjaf.mjs';
...
import { d as docsSchema } from './index_DAzkDQn7.mjs';
  ...
  return {
    ...
    load: async ({
      ...
      if (skipCount > 0) {
        ...
        logger.warn(
          `The glob() loader cannot be used for files in ${s.bold("src/content")} when legacy mode is enabled.`
        );
        if (skipCount > 10) {
          logger.warn(
            `Skipped ${s.green(skippedFiles.length)} files that matched ${s.green(patternList)}.`
          );
        } else {
...
const collectionConfigParser = union([
  object({
...
object({
  collections: record(string(), collectionConfigParser)
});
